"""One bounded optimization of the strict Y-corner outer relaxation.

The strict contact and positive-gap premises are retained. Reported
...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'results/det5_y_corner_strict_gauge_optimum.json'

def main():
 if OUT.exists():print('Strict optimization archive exists; skipped');return
 ...
 aa=list(z3.parse_smt2_string(text));g=z3.Real('gauge_level');aa.extend((g>=0,g<=1));s=z3.Optimize();s.set(timeout=15000);s.add(*aa);obj=s.maximize(g)
 path=ROOT/'results/det5_y_corner_strict_optimum.smt2';path.write_text(s.sexpr());start=time.monotonic();st=s.check()
 r={'scope':'One bounded strict-outer-relaxation optimization; rational SAT point checked, no proof of an upper bound independently certified here.','source_path':str(p.relative_to(ROOT)),'source_sha256':sha(p.read_bytes()),'input_path':str(path.relative_to(ROOT)),'input_sha256':sha(path.read_bytes()),'status':str(st),'elapsed_seconds':time.monotonic()-start,'timeout_ms':15000,'z3_version':z3.get_version_string()}
