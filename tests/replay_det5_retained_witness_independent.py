"""Independent rational body and retained-chart replay, using only stdlib.
...
def replay(witness,frontier):
    ...
    need(len(finite)==10,'ten finite gauges')
    values={','.join(map(str,v)):str(sum(map(abs,coords(v)))/2) for v in finite}
    need(all(Q(g)>beta for g in values.values()),'all imposed finite gauges')
    ...
    for perm in permutations(range(4)):
        ...
        symmetry_count+=1; v=tuple(map(int,multiply(A,(3,1,0))))
        if next(x for x in v if x)<0:v=tuple(-x for x in v)
        orbit.add(v)
    expected_orbit={(1,-1,1),(2,1,1),(3,1,0),(6,1,2)}
    need(symmetry_count==4 and orbit==expected_orbit,'complete affine contact symmetry orbit')
    need(not orbit.intersection(finite),'orbit gauges all absent from original ten')
    need(all(sum(map(abs,multiply(affine_inverse,[0,*v])))/2==Q(6,5) for v in orbit),'intrinsic orbit masses')
    ...
    return {'vertices':[[str(x) for x in v] for v in V],'volume':str(volume),'hollow_box':bbox,
            ...
            'finite_gauges':values,'missing_gauge_orbit':[list(v) for v in sorted(orbit)],'orbit_intrinsic_mass':'6/5','directional_charts':directions,'actual_free_heights':list(map(str,free)),
            'frontier_queries':len(frontier['queries']),'containing_pending_leaves':pending,'containing_closed_leaves':0}

def main():
    source=ROOT/'results/det5_joint_volume_gaps.json';front=ROOT/'results/det5_joint_round2.json'
    ...
    record={'status':'PASS','scope':'Actual hollow rational body occupies a retained normalized Y/U chart even with finite gauges and volume cuts; prevents closing this exact ten-gauge Y/U formulation, not a formulation with all necessary gauges or global widths.',
    ...
    (ROOT/'results/det5_retained_witness_validation.json').write_text(json.dumps(record,indent=2)+'\n')
